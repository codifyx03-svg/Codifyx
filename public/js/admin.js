  ...
  try {
    ...
    vettedData.workers.forEach(w => {
      ...
      const promoteBtn = isCore 
        ? `<button class="btn btn-secondary" style="background:#ef4444; border-color:#dc2626; padding:0.3rem 0.6rem; font-size:0.75rem; color:#fff" onclick="demoteWorkerFromCore(${w.id})">Demote</button>`
        : `<button class="btn btn-primary" style="background:#3b82f6; border-color:#2563eb; padding:0.3rem 0.6rem; font-size:0.75rem;" onclick="promoteWorkerToCore(${w.id})">Promote to Core</button>`;
    ...
    if (vettedData.workers.length === 0) {
      vettedTbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--text-muted)">No approved developers active yet.</td></tr>`;
    }
...
async function openWorkerProfile(workerId) {
  try {
    ...
    const actionBtn = s.is_core
      ? `<button class="btn btn-secondary" style="background:#ef4444; border-color:#dc2626; color:#fff; width:100%;" onclick="closeWorkerProfileModal(); demoteWorkerFromCore(${w.id})">Demote from Core Member</button>`
      : `<button class="btn btn-primary" style="background:#3b82f6; border-color:#2563eb; color:#fff; width:100%;" onclick="closeWorkerProfileModal(); promoteWorkerToCore(${w.id})">Promote to Core Member</button>`;
...
function openProjectManagementDetails(projectId) {
  ...
  p.tasks.forEach(t => {
    ...
    if (!t.assigned_worker_id) {
      ...
      assignBtnHTML = `<div style="font-size:0.8rem; color:var(--text-muted)">Assigned: <strong style="color:#fff">${t.worker_name || 'Vetted Developer'}</strong></div>`;
    }
...
async function openAssignTaskModal(taskId) {
  ...
  try {
    ...
    data.workers.forEach(w => {
      ...
      opt.value = w.id;
      opt.innerText = `${w.name} (${w.skills || 'General Dev'})`;
...
async function loadPaymentControl() {
  const tbody = document.getElementById('payment-control-table-body');
  tbody.innerHTML = '<tr><td colspan="6" style="text-align:center">Loading review queue...</td></tr>';
  ...
  try {
    ...
    data.projects.forEach(p => {
      if (p.tasks) {
        p.tasks.forEach(t => {
          // Show tasks that require approval (status review)
          if (t.status === 'review' || t.status === 'completed') {
            const tr = document.createElement('tr');
            
            let btnHTML = '';
            if (t.status === 'review') {
              btnHTML = `<button class="btn btn-primary" onclick="openReviewModal(${t.id}, '${t.title}', '${t.worker_name || 'Developer'}', \`${t.code_submission || ''}\`)">Audit Code</button>`;
            } else {
              btnHTML = `<span style="color:var(--status-completed); font-size:0.85rem">Disbursed (₹${t.payment_amount.toLocaleString('en-IN')})</span>`;
            }

            tr.innerHTML = `
              <td><strong>${p.title}</strong></td>
              <td>${t.title}</td>
              <td>${t.worker_name || 'Vetted Developer'}</td>
    ...
    if (tbody.innerHTML === '') {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:var(--text-muted)">No code deliverables requiring review.</td></tr>`;
    }
...
async function rejectSubmission() {
  ...
  if (!reason) {
    showToast('Please provide feedback explaining requested modifications', 'warning');
    return;
  }

  try {
    const response = await fetch(`/api/admin/tasks/${taskId}/reject-payment`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${getToken()}`
