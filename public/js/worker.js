...
async function loadEarningsHistory() {
  ...
  try {
    ...
    document.getElementById('earnings-approved-val').innerText = `₹${data.totalEarnings.toLocaleString('en-IN')}`;
    document.getElementById('earnings-pending-val').innerText = `₹${data.pendingPayments.toLocaleString('en-IN')}`;
    ...
    data.history.forEach(row => {
      ...
      tr.innerHTML = `
        <td><strong>${row.project_title}</strong></td>
        <td>${row.title}</td>
        <td>₹${row.payment_amount.toLocaleString('en-IN')}</td>
        <td>${dateStr}</td>
        <td><span class="status-badge ${row.payment_status.replace(' ', '-')}">${row.payment_status}</span></td>
      `;
      tbody.appendChild(tr);
    });

    if (data.history.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:var(--text-muted)">No transaction histories found. Claim and complete milestones to start earning.</td></tr>`;
    }
