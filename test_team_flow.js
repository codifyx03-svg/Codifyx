...
async function main() {
  ...
  const projectUrl = `${base}/api/projects/${projectId}`;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const response = await fetch(`${base}/api/projects/${projectId}/express-interest`, {
      method: 'POST',
  ...
  const groupsResponse = await fetch(`${base}/api/worker/groups`, {
  ...
  const projectCheck = await fetch(`${base}/api/admin/projects`, {
    headers: { 'Authorization': `Bearer ${tokens.admin}` }
  });
...
async function approveWorker(adminToken, email) {
  const workersRes = await fetch(`${base}/api/admin/workers/pending`, {
  ...
  const res = await fetch(`${base}/api/admin/workers/${worker.id}/approve`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${adminToken}` }
  });
  const json = await res.json();
