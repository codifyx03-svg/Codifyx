 ...
 */
async function _auditFinancial(eventType, referenceId, amount, actorId, details) {
...
async function _creditWallet(userId, walletType, amount) {
  await database.run(
    `UPDATE wallets
        SET balance = balance + ?,
...
async function _debitWallet(userId, walletType, amount) {
...
async function ensureWallet(userId, walletType) {
...
const PayoutService = {
  ...
  async fundProjectWallet(projectId, amount, actorId) {
    await ensureWallet(PLATFORM_UID, 'platform');
    await _creditWallet(PLATFORM_UID, 'platform', amount);
    ...
    await _auditFinancial('project_funded', projectId, amount, actorId, { projectId, companyFee, workerReserve });
  },
  ...
  async releaseTaskPayment(taskId, adminId) {
    ...
    await _debitWallet(PLATFORM_UID, 'platform', amount);
    await _creditWallet(workerId, 'worker', amount);
    ...
    await database.run(
      `UPDATE tasks SET status = 'completed', payment_status = 'released', completed_at = CURRENT_TIMESTAMP WHERE id = ?`,
      [taskId]
    );
  ...
  async approveWithdrawal(withdrawalId, adminId) {
    ...
    await ensureWallet(worker_id, 'worker');
    await _debitWallet(worker_id, 'worker', amount);
    ...
    // TODO (Phase 2 — Razorpay): await razorpayClient.payouts.create({ ... })
