...
const { logAuditEvent, logSecurityEvent } = require('../shared/audit');
...
app.get('/api/admin/payment/queue', authenticateAdminToken, authorizeAdminRoles('finance'), async (req, res) => {
  try {
    const tasks = await database.query(`
      SELECT t.id, t.title, t.payment_amount, t.payment_status, t.status,
             p.title as project_title,
             u.name as worker_name, u.email as worker_email
        FROM tasks t
        JOIN projects p ON p.id = t.project_id
        LEFT JOIN users u ON u.id = t.assigned_worker_id
       WHERE t.status = 'review' AND t.payment_status = 'pending'
       ORDER BY t.id DESC
    `);
