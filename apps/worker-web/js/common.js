...
const API_BASE = 'https://codifyx-public-api.onrender.com';
// JWT token helpers
function getToken() {
...
function connectChatWS(onMessageReceived, onSentConfirm) {
  ...
  let wsUri;
  if (API_BASE) {
    wsUri = API_BASE.replace(/^http/, 'ws').replace(/\/+$/, '') + '/api/messages';
  } else {
    const loc = window.location;
    wsUri = (loc.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + loc.host + '/api/messages';
  }
...
function downloadDocumentAsPDF(filename, content) {
  ...
  const textLines = doc.splitTextToSize(content, 520);
  doc.text(textLines, 40, 60);
  doc.save(filename);
}

async function downloadDocumentAsDocx(filename, content) {
  if (!window.docx || !window.docx.Document || !window.docx.Packer) {
    showToast('DOCX export library is not loaded.', 'error');
    return;
  }
