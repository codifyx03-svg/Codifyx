...
function logout() {
  ...
  window.location.href = 'portal-entry-secure-x97.html';
}
...
function checkAuth(allowedRoles = []) {
  ...
  if (!token || !user) {
    ...
    if (page && page !== 'portal-entry-secure-x97.html') {
      window.location.href = 'portal-entry-secure-x97.html';
    }
    return null;
  }
...
if (window.fetch) {
  ...
  window._rawFetch = originalFetch; // Exposed so login page can bypass the 401 interceptor
  window.fetch = async (input, init) => {
    ...
    if (url.startsWith('/api/')) {
      let base = API_BASE;
      if (url.startsWith('/api/messages') && typeof API_BASE_PUBLIC !== 'undefined' && API_BASE_PUBLIC) {
        base = API_BASE_PUBLIC;
      }
      if (base) {
    ...
    if (response.status === 401) {
      // Token auth disabled for local admin access debugging.
      // logout();
...
function connectChatWS(onMessageReceived, onSentConfirm) {
  ...
  let wsUri;
  const basePublic = (typeof API_BASE_PUBLIC !== 'undefined' && API_BASE_PUBLIC) ? API_BASE_PUBLIC : API_BASE;
  if (basePublic) {
    wsUri = basePublic.replace(/^http/, 'ws').replace(/\/+$/, '') + '/api/messages';
  } else {
    const loc = window.location;
    const host = loc.host === 'localhost:3002' ? 'localhost:3003' : loc.host;
    // Admin dashboard should connect directly to public API WS on localhost:3003 for local testing.
    wsUri = (loc.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + host + '/api/messages';
  }
